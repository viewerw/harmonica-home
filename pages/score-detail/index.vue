  ...
  <div class="container">
    <div class="detail">
      ...
      <view
        ...
        v-if="scoreDetail.harmonicaType"
      >
        <view class="action">
      ...
      <view class="flex flex-wrap">
        <view
          class="padding-xs"
          v-for="(item, index) in scoreDetail.harmonicaType"
          :key="index"
        >
          <view class="cu-tag light" :class="'bg-' + color[index]">{{
            item
          }}</view>
        </view>
      </view>
      <view
        class="cu-bar bg-white solid-bottom"
        v-if="scoreDetail.imgs && scoreDetail.imgs.length > 0"
      >
        <view class="action">
      ...
      <div class="score-text padding-sm text-df">
        {{ scoreDetail.uper }}({{ scoreDetail.date }})
      </div>
      ...
      <div v-if="scoreDetail.description" class="score-text padding-sm text-df">
...
export default {
  ...
  async mounted() {
    ...
    try {
      const { data } = await db.collection("songscore").doc(id).get();
